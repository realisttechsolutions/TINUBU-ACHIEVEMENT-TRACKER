...
import { saveTimelineEvent } from '@/server/admin/records-service';
import { timelineEventSchema } from '@/server/admin/validation';
  ...
  try {
    ...
    if (staffUser.role !== 'super_admin' && staffUser.role !== 'researcher') {
      return NextResponse.json(
        { error: 'Forbidden: Insufficient role permissions to manage timeline events' },
        { status: 403 },
      );
    }
    ...
    const parsedBody = timelineEventSchema.safeParse(body);
    if (!parsedBody.success) {
      return NextResponse.json(
        { error: 'Invalid timeline payload', details: parsedBody.error.flatten() },
        { status: 400 },
      );
    }

    const result = await saveTimelineEvent(id, parsedBody.data, staffUser);
    ...
    console.error('Error in POST /api/admin/records/[id]/timeline:', err);
