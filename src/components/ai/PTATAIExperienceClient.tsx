...
export const PTATAIExperienceClient: React.FC = () => {
  ...
  // Scroll listener to detect if user is scrolled away from latest content
  const handleScroll = useCallback(() => {
    const el = mainScrollRef.current;
    if (!el) return;
    const distanceToBottom = el.scrollHeight - el.scrollTop - el.clientHeight;
    // Show 'Latest' pill only when user has scrolled meaningfully up (> 250px)
    setUserScrolledUp(distanceToBottom > 250);
  }, []);
  ...
  const handleSendQuestion = async (question: string) => {
    ...
    // Submission Anchor: position the viewport to the new user question and top of assistant response
    setTimeout(() => {
      const userElem = document.getElementById(`msg-${userMessageId}`);
      if (userElem) {
        const prefersReducedMotion =
          typeof window !== 'undefined' &&
          window.matchMedia('(prefers-reduced-motion: reduce)').matches;
          ...
          for (const ev of events) {
            ...
            try {
              ...
              if (eventType === 'status') {
                ...
                // NATURAL READING PRINCIPLE: DO NOT auto-scroll to bottom while text streams.
                // The reader remains anchored at the start of the answer and scrolls down naturally.
                ...
                // COMPLETION: DO NOTHING TO SCROLL POSITION. Visually calm completion.
              }
            } catch {
  ...
  return (
    <div className="flex-1 flex overflow-hidden relative bg-slate-950 text-slate-100 font-sans selection:bg-cyan-500/30 selection:text-cyan-200 h-full">
      ...
      >
        ...
        <div className="flex-1 w-full max-w-2xl lg:max-w-3xl mx-auto px-3.5 sm:px-6 py-3 sm:py-5 flex flex-col relative z-10">
          ...
          {messages.length === 0 ? (
            ...
            <div className="flex-1 space-y-4 pb-32 sm:pb-36">
        ...
        {userScrolledUp && messages.length > 0 && (
          <button
            ...
            aria-label="Scroll to latest content"
            className="fixed bottom-20 right-6 z-30 inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-cyan-900/90 hover:bg-cyan-800 border border-cyan-500/50 text-cyan-100 text-xs font-medium shadow-2xl backdrop-blur-md transition-all hover:scale-105 active:scale-95"
          >
        ...
        {messages.length > 0 && (
          <div className="sticky bottom-0 inset-x-0 bg-gradient-to-t from-slate-950 via-slate-950/95 to-transparent pt-2 pb-3 sm:pb-4 px-3 sm:px-6 z-20">
