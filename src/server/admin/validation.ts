...
// Update Record Overview Schema
export const updateRecordOverviewSchema = createRecordSchema.extend({
  expected_updated_at: z.string().datetime({ offset: true }).or(z.string().min(10)),
});
...
export const listAdminRecordsSchema = z.object({
  ...
  publication_status: z.enum(['draft', 'in_review', 'ready_for_publication', 'published', 'rejected', 'archived', 'corrected']).optional(),
